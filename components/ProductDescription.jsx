...
const ProductDescription = ({ product }) => {
  ...
  return (
    <div className="my-18 text-sm dark:text-slate-200 text-slate-600">
      {/* Tabs */}
      <div className="flex border-b border-slate-200 dark:border-slate-700 mb-6 max-w-2xl">
        {["Description", "Reviews"].map((tab, index) => (
          <button
            className={`${
              tab === selectedTab
                ? "border-b-[1.5px] font-semibold dark:border-slate-600"
          ...
          {product.rating.map((item, index) => (
            <div key={index} className="flex gap-5 mb-10">
              ...
              <div>
                ...
                <p className="font-medium dark:text-slate-200 text-slate-800">
                  {item.user.name}
                </p>
                <p className="mt-3 font-light dark:text-slate-400 text-slate-600">
